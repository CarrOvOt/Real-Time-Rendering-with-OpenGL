...
更新：这里有个bug，从文件载入贴图的时候没有对基础颜色贴图（漫反射贴图）进行色彩空间变换/gamma校正，所以颜色变得非常黯淡，后面的版本修复了这个bug。
...
完善一下各种类的析构避免内存泄漏，主要是一些指针和GL的buffer以及Shader程序。

GL的buffer/shader program都使用一个unsigned int类型的变量充当指针，然而并没有智能指针之类的特性，如果一个类（比如mesh类）析构时将这个buffer删除，那么拷贝构造将会导致多个实例使用该buffer，其中一个实例析构时剩下的实例就会出问题。

这里目前想到两种办法：

第一种是禁用拷贝构造，在vector中使用指针存储多个对象（vector中类型必须是可拷贝的）

> T must meet the requirements of CopyAssignable and CopyConstructible.	(until C++11)

第二种是拷贝构造时再次绑定buffer

重新绑定buffer消耗应该是非常大的，我们考虑第一种方法，并使用shared_ptr管理指针。

- 尽量保持哪个类完成的buffer分配，哪个类就负责该buffer的释放。

- 如果某个类直接管理buffer，由该类负责释放，如Light类中的VBO，VAO。
- Model类和Mesh类同时含有vector\<Texture\>，由Model类负责释放GLTexture，Texture单纯作为结构体不负责buffer的分配和释放。

由于禁用了拷贝构造，原来类中的成员变量类型大部分要换成指针类型。
...
[禁止拷贝构造，禁止bug_后台服务器开发的博客-CSDN博客_禁止拷贝构造](https://blog.csdn.net/Travelerwz/article/details/109253393)

[转:STL容器里存放对象还是指针 - youxin - 博客园 (cnblogs.com)](https://www.cnblogs.com/youxin/p/3281339.html)
